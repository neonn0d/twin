...
export function resolveVault(name: string): string {
  if (!name.endsWith(".md")) name += ".md";
  const p = path.resolve(VAULT_PATH, name);
  if (!p.startsWith(path.resolve(VAULT_PATH))) throw new Error("Path traversal blocked");
  return p;
}
...
export function parseFrontmatter(content: string): { fm: Record<string, unknown>; body: string } {
  if (content.startsWith("---\n")) {
    ...
    if (end !== -1) {
      try { return { fm: (parseYaml(content.slice(4, end)) as Record<string, unknown>) || {}, body: content.slice(end + 5) }; }
      catch { /* ignore */ }
    }
  }
...
export function ensureSessionFile(cwd: string): string {
...
export function toStr(v: unknown): string {
  ...
  if (typeof v === "object" && v !== null) return Object.values(v as Record<string, unknown>).map(String).join(" \u2014 ");
  return String(v);
}
...
export function parseBrainPatterns(body: string): Record<string, [string, string]> {
  ...
  for (const line of body.split("\n")) {
    ...
    if (fm) {
      if (inFence) { if (name) patterns[name] = [lang, lines.join("\n").trim()]; inFence = false; lines.length = 0; }
