...
server.setRequestHandler(CallToolRequestSchema, async (request: any) => {
  ...
  const h: Record<string, (p: Record<string, unknown>) => string> = {
    ...
    get_knowledge: getKnowledge, list_knowledge: listKnowledge, search_knowledge: searchKnowledge,
  ...
  const fn = h[name];
  if (!fn) return { content: [{ type: "text", text: `Unknown tool: ${name}` }] };
  try { return { content: [{ type: "text", text: fn(p) }] }; }
  catch (err) { return { content: [{ type: "text", text: `Error: ${err instanceof Error ? err.message : String(err)}` }] }; }
});
